...
const apiBase = import.meta.env.VITE_API_URL;   // ✅ use env var instead of localhost
...
const Chat = () => {
  ...
  useEffect(()=>{
    if(roomid){
      axios.get(`${apiBase}/room/${roomid}`).then((response)=>{
        setRoomname(response.data.name);
        setUpdatedAt(response.data.updatedAt);
      })
      axios.get(`${apiBase}/messages/${roomid}`).then((response)=>{
  ...
  const Sendmessage = async(e) =>{
    ...
    await axios.post(`${apiBase}/messages/new`,{   // ✅ use apiBase here too
      message:input,
      name:user.displayName,   // make sure this matches Firebase user object
      timestamp:new Date(),
      uid:user.uid,
      roomid: roomid,
    });
